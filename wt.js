...
async function initiateHandshake() {
  try {
    ...
    // Set QUIC version to the one expected by the server (e.g., 0x00000001 for RFC 9000)
    view.setUint32(0, 0x00000001, true); // true for little-endian
    // Role (ROLE_SUBSCRIBER): 0x02
    view.setUint32(4, 0x02, true); // Assuming uint32 for role

    await writer.write(buffer);
    writer.releaseLock();
    logMessage("Handshake initiated with role set as subscriber and correct QUIC version.");
    console.log("Handshake initiated with role set as subscriber and correct QUIC version.");
  } catch (error) {
